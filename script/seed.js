...
async function seed() {
  ...
  const users = await Promise.all([
    ...
    User.create({ userName: 'Darren', password: '123' }),
    User.create({ userName: 'Deepak', password: '123' }),
    User.create({ userName: 'Jeff', password: '123' }),
    User.create({ userName: 'H', password: '123' }),
    User.create({ userName: 'Simmons', password: '123' }),
  ])
...
const events = await Promise.all([
    ...
    Event.create({ name: 'Wall Balls', description: "100", image: "https://www.cycfitness.co.uk/uploads/products/gallery/hold-strong-hyrox-wall-ball-1643822493-1280.jpg", targetTime: "3:45" }).then(event => eventMap.set(event.name, event)),
